...
func (c *ControllerV1) ItemDownload(ctx context.Context, req *v1.ItemDownloadReq) (res *v1.ItemDownloadRes, err error) {
	...
	span.SetAttributes(
		...
		attribute.String("request.parameter.share.first_4_chars", req.DeviceShare[:min(4, len(req.DeviceShare))]),
...
`, ac.Id, req.ItemId).Scan(&sqlV)
	...
	if path, err := c.fu.ItemExits(sqlV.Filename); err != nil {
		span.SetStatus(codes.Error, "检测到用户寻找的文件不存在")
		span.SetAttributes(
			attribute.String("file.extract.error", err.Error()),
			attribute.String("file.expected.path", path),
		)

		return nil, gerror.NewCode(gcode.CodeNotFound, "条目不存在")
	}
...
from public.shares where item_id = ? and share_type = ? and status = ? and user_id = ?`, sqlV.ItemId, dao.ShareTypeAuth, dao.ShareStatusActive, ac.Id).Scan(&sqlV)
	...
	*/
	// spew.Dump(sqlV)
	sqlV.decodedAuthShare, err = shamir.ShareFromBase64(sqlV.AuthShare)
	if err != nil {
